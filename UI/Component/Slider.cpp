...
void Slider::Draw() const {
    ...
    End1.Draw();
    End2.Draw();
    ImageButton::Draw();    // button on the bar
}
void Slider::SetOnValueChangedCallback(std::function<void(float value)> onValueChangedCallback) {
    // behavior is determined by caller
...
void Slider::SetValue(float value) {
    ...
    Position.x = Bar.Position.x + Bar.Size.x * this->value; // update slider position
    // will call SettingsScene::BGM/SFXSlideOnValueChanged(float)
